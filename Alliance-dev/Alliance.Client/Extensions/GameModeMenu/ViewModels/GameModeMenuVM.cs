...
namespace Alliance.Client.Extensions.GameModeMenu.ViewModels
{
	public class GameModeMenuVM : ViewModel
	{
        ...
        private List<SelectionItem> GetFactionChoices()
        {
            ...
            foreach (string factionId in Factions.Instance.AvailableCultures.Keys)
            {
                // Get the readable faction name from AvailableCultures dictionary (BasicCultureObject)
                string readableName = Factions.Instance.AvailableCultures.TryGetValue(factionId, out var cultureObj)
                                      ? cultureObj.Name.ToString()
                                      : factionId;

                choices.Add(new SelectionItem(false, factionId, readableName));
            }
            return choices;
        }
