...
{
    public class ALBehaviorMPLightInfantry : BehaviorComponent
    {
        ...
        private const float MaxLateralOffset = 34f;
        public ALBehaviorMPLightInfantry(Formation formation)
            : base(formation)
        {
        ...
        protected override void CalculateCurrentOrder()
        {
            ...
            if (Formation.Team.FormationsIncludingEmpty.AnyQ((f) => f.CountOfUnits > 0 && f != Formation && f.QuerySystem.IsInfantryFormation))
            {
                ...
                if (flag)
                {
                    if (Formation.QuerySystem.ClosestSignificantlyLargeEnemyFormation.MedianPosition.AsVec2.DistanceSquared(Formation.QuerySystem.AveragePosition) <= 10000f)
                    {
         ...
         Formation.QuerySystem.ClosestSignificantlyLargeEnemyFormation.MedianPosition.AsVec2
                 .DistanceSquared(Formation.QuerySystem.AveragePosition) >= 900f)
            {
                Vec2 enemyPos = Formation.QuerySystem.ClosestSignificantlyLargeEnemyFormation.MedianPosition.AsVec2;
                ...
                // scale offset down if very close
                ...
                // convert to Vec3 using terrain height
        ...
        public override void TickOccasionally()
        {
            ...
            FormationBehaviorHelper.GetBestTargetFormation(Formation, FormationClass.Skirmisher);

            var enemyFS = Formation.QuerySystem.ClosestSignificantlyLargeEnemyFormation;
            if (enemyFS == null)
                return;
            Agent closestEnemy = Formation.QuerySystem.ClosestEnemyAgent;
            ...
            Vec2 closestDistance = closestEnemy.Position.AsVec2;
        ...
        {
            if (Formation.FormationIndex == FormationClass.Skirmisher)
            {
                return 1.2f;
            }
            return 0f;
