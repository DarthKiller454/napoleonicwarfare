...
namespace Alliance.Server.Extensions.Vehicles.Handlers
{
    public class VehicleHandler : IHandlerRegister
    {
            ...
            public void Register(GameNetwork.NetworkMessageHandlerRegisterer reg)
            {
                ...
                reg.Register<CS_VehicleRequestBrake>(HandleRequestBrake);
            ...
            public static bool HandleRequestBrake(NetworkCommunicator peer, CS_VehicleRequestBrake message)
            {
                MissionObject vehicle = Mission.MissionNetworkHelper.GetMissionObjectFromMissionObjectId(message.MissionObjectId);
                CS_Vehicle vehicleScript = vehicle?.GameEntity?.GetFirstScriptOfType<CS_Vehicle>();

                if (vehicleScript != null && peer.ControlledAgent == vehicleScript.PilotAgent)
                {
                    vehicleScript.ServerApplyBrake();

                    GameNetwork.BeginBroadcastModuleEvent();
                    GameNetwork.WriteMessage(new CS_VehicleSyncBrake(message.MissionObjectId));
