...
namespace Alliance.Common.Extensions.UsableEntity.Behaviors
{
	...
	{
		public List<GameEntity> _usableEntities = new List<GameEntity>();
		...
		public void HideEntity(GameEntity entity)
		{
            ...
            foreach (Agent agent in Mission.Current.Agents)
            {
                ...
                // Check if the agent is using a game object
                if (agent.IsUsingGameObject)
                {
                    GameEntity usedEntity = agent.CurrentlyUsedGameObject.GameEntity;

                    // Check if it's invalid (removed, disabled, etc.)
		...
		public void RegisterUsableEntity(GameEntity entity)
        {
            if (entity != null && !_usableEntities.Contains(entity))
            {
