	...
	public class PBVoiceChatHandlerClient : MissionNetwork
	{
		...
		public override void OnBehaviorInitialize()
		{
			...
			if (GameNetwork.IsClient)
			{
                ...
                {
                    Log("No audio output devices available. VOIP disabled.", LogLevel.Warning);
                    _isVoiceChatDisabled = true;
                    return;
                }
                ...
                if (WaveIn.DeviceCount > 0)
                {
                    ...
                    {
                        DeviceNumber = -1, // default input device
                ...
                {
                    DeviceNumber = -1 // default output device
                };
        ...
        public override void OnRemoveBehavior()
        {
            ...
            {
                Log($"VOIP cleanup error: {ex}", LogLevel.Warning);
            }
        ...
        private void HandleOptionsUpdated()
        {
            try
            {
                ...
                if (WaveOut.DeviceCount == 0)
                {
                    Log("No audio output devices found after options update.", LogLevel.Warning);
                    _isVoiceChatDisabled = true;
                    return;
                }

                // Recreate input device
                if (WaveIn.DeviceCount > 0)
                {
                    _waveIn = new WaveInEvent
                    {
            ...
            {
                Log($"VOIP device reinitialization failed: {ex}", LogLevel.Error);
                _isVoiceChatDisabled = true;
            }
        }
