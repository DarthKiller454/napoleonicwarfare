	...
	class Patch_MissionNetworkComponent
	{
		...
		public static bool Patch()
		{
			try
			{
				...
				_patched = true;
				Harmony.Patch(
					typeof(MissionNetworkComponent).GetMethod("HandleServerEventSetWieldedItemIndex",
						BindingFlags.Instance | BindingFlags.NonPublic),
					prefix: new HarmonyMethod(typeof(Patch_MissionNetworkComponent).GetMethod(
						nameof(Prefix_HandleServerEventSetWieldedItemIndex), BindingFlags.Static | BindingFlags.Public)));
				Harmony.Patch(
		...
		// Trigger ExtendedItemView.OnAgentWieldedItemChange
		public static bool Prefix_HandleServerEventSetWieldedItemIndex(GameNetworkMessage baseMessage)
		{
			...
			if (agentFromIndex != null)
			{
				...
				// Handle the wielding of ExtendedItems
				Mission.Current.GetMissionBehavior<ExtendedItemView>()?.OnAgentWieldedItemChange(agentFromIndex);
			}
