...
namespace Alliance.Common.GameModels
{
	public class ExtendedAgentApplyDamageModel : CustomAgentApplyDamageModel
    {
		...
		public override float ApplyGeneralDamageModifiers(in AttackInformation attackInformation, in AttackCollisionData collisionData, float baseDamage)
		{
			...
			if(currentUsageItem != null && currentUsageItem.IsMeleeWeapon && missionWeapon.Item.StringId.Contains("bandage") && attackInformation.VictimAgent != null && attackInformation.VictimAgent.IsActive())
			{
				float victimHealth = attackInformation.VictimAgent.Health;
				float victimMaxHealth = attackInformation.VictimAgent.HealthLimit;
				float healing = 20f;
				healing += victimHealth;
                healing = Math.Min(healing, victimMaxHealth);
				attackInformation.VictimAgent.Health = healing;
                baseDamage = 0f;
			}
			return baseDamage;
		}
