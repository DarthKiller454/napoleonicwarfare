...
namespace Alliance.Common.Extensions.Artillery
{
    ...
    {
        public PE_MangonelAI(NWF_Cannon_Moveable mangonel) : base(mangonel)
        {
        }
    }
    public class NWF_Cannon_Moveable : RangedSiegeWeapon, ISpawnable, IMoveable
    {
        ...
        {
            if(base.GameEntity.HasTag("placedbyscript"))
            { 
        ...
        protected override void OnMissionReset()
        {
            ...
            foreach (StandingPointWithWeaponRequirement ammoPickUpStandingPoint in AmmoPickUpPoints)
            {
        ...
        public virtual MatrixFrame SyncFrame(MatrixFrame frame)
        {
            ...
            if (Mission.Current == null || Mission.Current.IsMissionEnding)
                return frame;

            MatrixFrame current = GameEntity.GetFrame();

            if (!frame.origin.NearlyEquals(current.origin) ||
                !frame.rotation.NearlyEquals(current.rotation))
            {
        ...
        {
            this.DefaultSide = BattleSideEnum.None;
            ...
            if (this.StandingPoints != null)
            {
                ...
                {
                    this._standingPointLocalIKFrames[i] = this.StandingPoints[i].GameEntity.GetGlobalFrame().TransformToLocal(in globalFrame);
            ...
            this.moverStandingPoint = base.GameEntity.GetFirstChildEntityWithTagRecursive(this.MoverStandingPointTag).GetFirstScriptOfType<StandingPoint>();
        ...
        {
            return State == NWF_Cannon_Moveable.WeaponState.Idle || State == NWF_Cannon_Moveable.WeaponState.LoadingAmmo || State == NWF_Cannon_Moveable.WeaponState.WaitingBeforeIdle;
        }
        ...
        protected void MoveControl()
        {
            ...
            {
                if (this.GetPilotAgent() != null || this.GetRotationAgent() != null)
                {
                    if (this.GetPilotAgent() != null)
                    {
                        if (this.GetPilotAgent().Position.Distance(base.GameEntity.GlobalPosition) > 2.65f)
                        {
                    ...
                    {
                        if (this.GetRotationAgent().Position.Distance(base.GameEntity.GlobalPosition) > 2.65f)
                        {
            ...
            {
                if (Agent.Main != null && this.GetPilotAgent() == Agent.Main && this.GetRotationAgent() == null)
                {
            ...
            if (!GameNetwork.IsClientOrReplay)
            {
                foreach (StandingPointWithWeaponRequirement standingPointWithWeaponRequirement in this.AmmoPickUpPoints)
                {
                    if (standingPointWithWeaponRequirement.HasUser)
                    {
                        ...
                        if (!(currentAction == NWF_Cannon_Moveable.act_pickup_boulder_begin))
                        {
                            if (currentAction == NWF_Cannon_Moveable.act_pickup_boulder_end)
                            {
        ...
        protected override void OnTickParallel(float dt)
        {
            ...
            if (State == NWF_Cannon_Moveable.WeaponState.WaitingBeforeProjectileLeaving)
            {
                this.UpdateProjectilePosition();
            }
            ...
            for (int i = 0; i < this.StandingPoints.Count; i++)
            {
                if (this.StandingPoints[i].HasUser)
                {
                    ...
                    else if (this.StandingPoints[i] != base.PilotStandingPoint)
                    {
                        ...
                        {
                            this.StandingPoints[i].UserAgent.SetHandInverseKinematicsFrameForMissionObjectUsage(this._standingPointLocalIKFrames[i], boundEntityGlobalFrame, 0f);
                        }
                        else
                        {
                            this.StandingPoints[i].UserAgent.ClearHandInverseKinematics();
            ...
            if (!GameNetwork.IsClientOrReplay)
            {
                for (int j = 0; j < this._rotateStandingPoints.Count; j++)
                {
                    ...
                    if (standingPoint.HasUser && !standingPoint.UserAgent.SetActionChannel(1, (j == 0) ? this._rotateLeftAnimationActionIndex : this._rotateRightAnimationActionIndex, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true) && standingPoint.UserAgent.Controller != AgentControllerType.AI)
                    {
                ...
                if (base.PilotAgent != null)
                {
                    ...
                    if (State == NWF_Cannon_Moveable.WeaponState.WaitingBeforeProjectileLeaving)
                    {
                        ...
                        {
                            if (currentAction != ActionIndexCache.act_none && currentAction != NWF_Cannon_Moveable.act_strike_bent_over)
                            {
                                base.PilotAgent.SetActionChannel(1, NWF_Cannon_Moveable.act_strike_bent_over, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true);
                            }
                        }
            ...
            if (State == NWF_Cannon_Moveable.WeaponState.Reloading)
            {
                foreach (StandingPoint standingPoint2 in this.ReloadStandingPoints)
                {
                    if (standingPoint2.HasUser)
                    {
                        ...
                        else if (!GameNetwork.IsClientOrReplay)
                        {
                            ...
                            if (!standingPoint2.UserAgent.SetActionChannel(1, in actionIndexCache, ignorePriority: false, (AnimFlags)0UL, 0f, 1f, -0.2f, 0.4f, this._bodySkeleton.GetAnimationParameterAtChannel(0), false, -0.2f, 0, true) && standingPoint2.UserAgent.Controller != AgentControllerType.AI)
                            {
        ...
        protected override void OnRangedSiegeWeaponStateChange()
        {
            ...
            NWF_Cannon_Moveable.WeaponState state = State;
            if (state == NWF_Cannon_Moveable.WeaponState.Shooting)
            {
            ...
            if (state != NWF_Cannon_Moveable.WeaponState.Idle)
            {
                if (state != NWF_Cannon_Moveable.WeaponState.Shooting)
                {
                    if (state == NWF_Cannon_Moveable.WeaponState.WaitingBeforeIdle)
                    {
                        this.UpdateProjectilePosition();
                        return;
                    }
        ...
        public override TextObject GetDescriptionText(WeakGameEntity gameEntity)
        {
            ...
            {
                return new TextObject(CannonName, null);
            }
            return new TextObject("Cannonball", null);
        }
