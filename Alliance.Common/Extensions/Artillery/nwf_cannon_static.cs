...
{
    public sealed class PE_MangonelAI_Static : RangedSiegeWeaponAi
    {
        public PE_MangonelAI_Static(NWF_Cannon_Static mangonel) : base(mangonel)
        {
        }
    }
    public class NWF_Cannon_Static : RangedSiegeWeapon, ISpawnable, IMoveable
    {
        ...
        {
            return new PE_MangonelAI_Static(this);
        }
        ...
        {
            return State == NWF_Cannon_Static.WeaponState.Idle || State == NWF_Cannon_Static.WeaponState.LoadingAmmo || State == NWF_Cannon_Static.WeaponState.WaitingBeforeIdle;
        }
            ...
            if (!GameNetwork.IsClientOrReplay)
            {
                foreach (StandingPointWithWeaponRequirement standingPointWithWeaponRequirement in this.AmmoPickUpPoints)
                {
                    if (standingPointWithWeaponRequirement.HasUser)
                    {
                        ...
                        if (!(currentAction == NWF_Cannon_Static.act_pickup_boulder_begin))
                        {
                            if (currentAction == NWF_Cannon_Static.act_pickup_boulder_end)
                            {
        ...
        protected override void OnTickParallel(float dt)
        {
            ...
            if (State == NWF_Cannon_Static.WeaponState.WaitingBeforeProjectileLeaving)
            {
                this.UpdateProjectilePosition();
            }
            ...
            if (!GameNetwork.IsClientOrReplay)
            {
                ...
                if (base.PilotAgent != null)
                {
                    ...
                    if (State == NWF_Cannon_Static.WeaponState.WaitingBeforeProjectileLeaving)
                    {
                        ...
                        {
                            if (currentAction != ActionIndexCache.act_none && currentAction != NWF_Cannon_Static.act_strike_bent_over)
                            {
                                base.PilotAgent.SetActionChannel(1, NWF_Cannon_Static.act_strike_bent_over, false, 0UL, 0f, 1f, -0.2f, 0.4f, 0f, false, -0.2f, 0, true);
                            }
                        }
            ...
            if (State == NWF_Cannon_Static.WeaponState.Reloading)
            {
                foreach (StandingPoint standingPoint2 in this.ReloadStandingPoints)
                {
        ...
        protected override void OnRangedSiegeWeaponStateChange()
        {
            ...
            NWF_Cannon_Static.WeaponState state = State;
            if (state == NWF_Cannon_Static.WeaponState.Shooting)
            {
            ...
            if (state != NWF_Cannon_Static.WeaponState.Idle)
            {
                if (state != NWF_Cannon_Static.WeaponState.Shooting)
                {
                    if (state == NWF_Cannon_Static.WeaponState.WaitingBeforeIdle)
                    {
                        this.UpdateProjectilePosition();
                        return;
                    }
