using Alliance.Common.Extensions.Artillery.CommonAIFunctions;
using System;
using System.Linq;
...
public static class CommonAIDecisionFunctions
{
    ...
    public static Func<Target, float> TargetDistanceToHostiles(Team team = null)
    {
        return target =>
        {
            ...
            {
                var querySystemClosestEnemyFormation = target.Formation.QuerySystem.ClosestSignificantlyLargeEnemyFormation;
                if (querySystemClosestEnemyFormation == null)
                {
                    return float.MaxValue;
                }

                return target.GetPositionPrioritizeCalculated().AsVec2.Distance(querySystemClosestEnemyFormation.WeightedAverageEnemyPosition);
            }
    ...
    {
        return target => target.Formation.QuerySystem.MovementSpeedMaximum;
    }
...
public static class CommonAIFunctions
{
    ...
    public static bool HasLineOfSight(Vec3 from, Vec3 to, float atLeast = 70)
    {
        ...
        {
            Mission.Current.Scene.RayCastForClosestEntityOrTerrain(from, to, out distanceE, out WeakGameEntity entity);
        }
